...
```{r}
tipnames %>% strsplit(.,"_") %>% lapply(.,tail,1) %>% unlist %>% as.double -> decdates
```
...
```{r}
h <- rep("DRC",length(tre.rtt$tip.label))
h[grep("SierraLeone",tre.rtt$tip.label)] <- "SierraLeone"
h[grep("Guinea",tre.rtt$tip.label)] <- "Guinea"
dd <- data.frame(taxa=tre.rtt$tip.label,Country=h)
g1 <- ggtree(tre.rtt) %<+% dd
p <- g1+theme_tree2()+geom_tippoint(aes(shape=Country, color=Country), alpha=1.0)+theme(legend.position="right")+scale_color_manual(values=c(c("DRC"="darkorange1","SierraLeone"="blue","Guinea"="red")))
p
```
...
In order to calculate the population size from the tree, we use `skyspline`, which fits a non-parametric model to the 'growth rate' of the epidemic, and generates an estimate of the population size, rather than an effective population size. This requires an estimate of the death/recovery rate of the infection. Let us assume it is 10 days.
...
tre.dated2.sky <- fit.skyspline.ml(tre.dated2, death_rate_guess = 1/(10/365), t0=t0, y0_guess=1,  R0guess = 1, np_range = 2)
```
...
```{r}
fit <- phylandml(tre2, delimiter=NULL, regex="DRC|SierraLeone|Guinea",quiet=TRUE)
```
